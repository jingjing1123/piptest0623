...
class  CourseAPI:
    ...
    def add_course(self,name,desc,displayidx,idSavedName=None):
        payload = {
            'action': 'add_course',
            # 格式化字符串的方式来构造消息
            'data':'''
        ...
        if idSavedName:
            BuiltIn().set_global_variable('${%s}'%idSavedName, retDict['id'])
            print(f"global var set: ${idSavedName}:{retDict['id']}")
        return retDict
